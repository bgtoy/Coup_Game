...
class CoupGame{
    ...
    listen() {
        this.players.map(x => {
            ...
            this.bindPlayerListeners(x, socket);
        })
    }

    // Attaches the full set of game-event listeners to a given socket.
    // Used both for the initial game start and for re-attaching a socket
    // after a player reconnects mid-game.
    bindPlayerListeners(player, socket) {
            let bind = this
            ...
            socket.on('g-actionDecision', (res) => {
                ...
                if(bind.players[bind.currentPlayer].name !== res.action.source) {
                    console.log(`ignored action from ${res.action.source}, not their turn`);
                    return;
                }
                if(!bind.actions[res.action.action]) {
                    console.log(`ignored unknown action ${res.action.action}`);
                    return;
                }
    ...
    // Called when a previously-connected player reconnects with a new socket
    // (e.g. after a dropped connection or page refresh). Re-binds all the
    // game listeners to the new socket and re-syncs that player's view of
    // the game so they can keep playing without restarting.
    reattachPlayer(name, newSocket) {
        const index = this.nameIndexMap[name];
        if (index === undefined) return false;

        this.players[index].socketID = newSocket.id;
        this.players[index].isConnected = true;
        this.nameSocketMap[name] = newSocket.id;

        this.bindPlayerListeners(this.players[index], newSocket);

        this.gameSocket.emit("g-addLog", `${name} reconnected`);

        // Re-sync this player's view of the world.
        this.updatePlayers();
        newSocket.emit('g-updateCurrentPlayer', this.players[this.currentPlayer].name);

        if (this.players[index].isDead) {
            return true;
        }

        // If it's currently this player's turn to choose an action, re-prompt them.
        if (this.currentPlayer === index && !this.isChallengeBlockOpen && !this.isRevealOpen
            && !this.isChooseInfluenceOpen && !this.isExchangeOpen) {
            newSocket.emit('g-chooseAction');
        }

        return true;
    }

    // Called when a player's socket disconnects mid-game. We don't remove
    // them from the game (their seat, coins, and influences are preserved)
    // so they can reattach later with the same player token.
    markDisconnected(name) {
        const index = this.nameIndexMap[name];
        if (index === undefined) return;
        this.players[index].isConnected = false;
        this.gameSocket.emit("g-addLog", `${name} disconnected`);
        this.updatePlayers();
    }
